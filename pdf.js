...
const parseWeekRange = (plain) => {
  ...
  const rangeMatch = plain.match(/tuan\s*(\d+)\s*[-~â€“]\s*(\d+)/);
  if (rangeMatch) {
...
const resolveInputPath = (arg) => {
  if (!arg) {
    return path.resolve(__dirname, 'tkb.pdf');
  }

  const candidateCwd = path.resolve(process.cwd(), arg);
  if (fs.existsSync(candidateCwd)) {
    return candidateCwd;
  }

  const candidateLocal = path.resolve(__dirname, arg);
...
const buildDayLayout = (items) => {
  ...
  if (periodItems.length === 0) {
    throw new Error('Khong tim thay thong tin tiet trong file PDF.');
  }
...
const parseLessonBlock = (text) => {
  ...
  lines.forEach((line) => {
    ...
    if (!result.teacher) {
      const teacherMatch = trimmed.match(/^GV[:ï¼š]?\s*(.+)$/i);
      if (teacherMatch) {
    ...
    if (plain.includes('nghi') && plain.includes('tuan')) {
      noteCandidate = true;
      const nghIndex = plain.indexOf('nghi');
      const offText = nghIndex >= 0 ? plain.slice(nghIndex) : plain;
      const offRange = parseWeekRange(offText);
      if (offRange) {
        offRange.weeks.forEach((week) => weeksOff.add(week));
      } else {
        extractNumbers(offText).forEach((num) => weeksOff.add(num));
      }
      handled = true;
    }
...
const sessionRankLookup = {
  [toPlainLower('SÃ¡ng')]: 0,
  [toPlainLower('Chiá»u')]: 1,
  [toPlainLower('Tá»‘i')]: 2,
};
...
const main = async () => {
  ...
  const pdfPath = resolveInputPath(inputArg);

  if (!fs.existsSync(pdfPath)) {
    console.error('Khong tim thay file PDF can xu ly.');
    console.error('Cach dung: node pdf.js [duong-dan-toi-file.pdf]');
    process.exit(1);
    return;
  }
  ...
  if (pageSchedules.size === 0) {
    console.warn('Khong tim thay du lieu thoi khoa bieu phu hop trong PDF.');
    return;
  }
  ...
  if (Object.keys(simplifiedSheets).length === 0) {
    console.warn('Khong tim thay du lieu thoi khoa bieu sau khi chuyen doi.');
    return;
  }
  ...
  const outputPath = path.join(
    ...
    `${path.basename(pdfPath, path.extname(pdfPath))}.schedule.json`
  );

  fs.writeFileSync(outputPath, JSON.stringify(scheduleOutput, null, 2), {
    encoding: 'utf8',
  });

  console.log(`Da xuat: ${path.basename(outputPath)}`);
};

main().catch((error) => {
  console.error('Khong xu ly duoc file PDF:', error);
  process.exitCode = 1;
});
